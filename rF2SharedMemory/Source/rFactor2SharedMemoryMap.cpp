...
Napi::Object InitAll(Napi::Env env, Napi::Object exports) {
    return SharedMemoryPlugin::SetExports(env, exports);
}

NODE_API_MODULE(NODE_GYP_MODULE_NAME, InitAll)

Napi::Object SharedMemoryPlugin::SetExports(Napi::Env env, Napi::Object exports)
{
    exports.Set("init", Napi::Function::New(env, SharedMemoryPlugin::Init));
    exports.Set("getTelemetry", Napi::Function::New(env, SharedMemoryPlugin::GetTelemetry));
    exports.Set("getScoring", Napi::Function::New(env, SharedMemoryPlugin::GetScoring));
    /*exports.Set("getPhysics", Napi::Function::New(env, SharedMemoryPlugin::GetPhysics));
    exports.Set("getGraphicsAssetto", Napi::Function::New(env, SharedMemoryPlugin::GetGraphicsAssetto));
    exports.Set("getGraphicsACC", Napi::Function::New(env, SharedMemoryPlugin::GetGraphicsACC));
    exports.Set("getStatics", Napi::Function::New(env, SharedMemoryPlugin::GetStatics)); */
    exports.Set("cleanup", Napi::Function::New(env, SharedMemoryPlugin::Cleanup));
    return exports;
}

SharedMemoryPlugin* smp;

Napi::Number SharedMemoryPlugin::Init(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    smp = new SharedMemoryPlugin();
    smp->Startup();
    return Napi::Number::New(env, 0);
}

Napi::Object SharedMemoryPlugin::GetTelemetry(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    Napi::Object ret = Napi::Object::New(env);
    if (smp && smp->mTelemetry.ReadUpdate()) {
        ...
        double speed = sqrt((telemetry.mLocalVel.x * telemetry.mLocalVel.x)
          ...
          + (telemetry.mLocalVel.z * telemetry.mLocalVel.z)) * 3.6;
        ret.Set("mSpeed", Napi::Number::New(env, speed));          

        ret.Set("mFuel", Napi::Number::New(env, telemetry.mFuel));

        double mTireTemps[4] {0};
        mTireTemps[0] = telemetry.mWheels[0].mTireCarcassTemperature;
        mTireTemps[1] = telemetry.mWheels[1].mTireCarcassTemperature;
        mTireTemps[2] = telemetry.mWheels[2].mTireCarcassTemperature;
        mTireTemps[3] = telemetry.mWheels[3].mTireCarcassTemperature;
        double mTireTemp = (mTireTemps[0] + mTireTemps[1] + mTireTemps[2] + mTireTemps[3]) / 4;
        ret.Set("mTireTemp", Napi::Number::New(env, mTireTemp));

        double mBrakeTemps[4] {0};
        mBrakeTemps[0] = telemetry.mWheels[0].mBrakeTemp;
        mBrakeTemps[1] = telemetry.mWheels[1].mBrakeTemp;
        mBrakeTemps[2] = telemetry.mWheels[2].mBrakeTemp;
        mBrakeTemps[3] = telemetry.mWheels[3].mBrakeTemp;
        double mBrakeTemp = (mBrakeTemps[0] + mBrakeTemps[1] + mBrakeTemps[2] + mBrakeTemps[3]) / 4;
        ret.Set("mBrakeTemp", Napi::Number::New(env, mBrakeTemp));

        double mPressures[4] {0};
        mPressures[0] = telemetry.mWheels[0].mPressure;
        mPressures[1] = telemetry.mWheels[1].mPressure;
        mPressures[2] = telemetry.mWheels[2].mPressure;
        mPressures[3] = telemetry.mWheels[3].mPressure;                 
        double mPressure = (mPressures[0] + mPressures[1] + mPressures[2] + mPressures[3]) / 4;
        ret.Set("mPressure", Napi::Number::New(env, mPressure));

        ret.Set("mEngineOilTemp", Napi::Number::New(env, telemetry.mEngineOilTemp));
...
Napi::Object SharedMemoryPlugin::GetScoring(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    Napi::Object ret = Napi::Object::New(env);
    if (smp && smp->mScoring.ReadUpdate()) {
        
        rF2VehicleScoring scoring = smp->mScoring.mReadBuff.mVehicles[smp->mCurrTelemetryVehicleIndex];
        ...
        ret.Set("mLastLapTime", Napi::Number::New(env, scoring.mLastLapTime));
        ret.Set("mBestLapTime", Napi::Number::New(env, scoring.mBestLapTime));
        ret.Set("mEstimatedLapTime", Napi::Number::New(env, scoring.mEstimatedLapTime));
        ret.Set("mTotalLaps", Napi::Number::New(env, scoring.mTotalLaps));
        ret.Set("mPlace", Napi::Number::New(env, scoring.mPlace));

        ret.Set("mUnderYellow", Napi::Number::New(env, scoring.mUnderYellow));

        ret.Set("mMaxLaps", Napi::Number::New(env, smp->mScoring.mReadBuff.mScoringInfo.mMaxLaps));
        ret.Set("mCurrentET", Napi::Number::New(env, smp->mScoring.mReadBuff.mScoringInfo.mCurrentET));
    }
    return ret;
}

Napi::Number SharedMemoryPlugin::Cleanup(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    if (smp) {
        smp->Shutdown();
    }
...
void SharedMemoryPlugin::Startup()
{
  ...
  InitMappedBuffer(mTelemetry, "Telemetry",  SubscribedBuffer::Telemetry);
  InitMappedBuffer(mScoring, "Scoring", SubscribedBuffer::Scoring);
  InitMappedBuffer(mRules, "Rules", SubscribedBuffer::Rules);
  InitMappedBuffer(mMultiRules, "Multi Rules", SubscribedBuffer::MultiRules);
  InitMappedBuffer(mForceFeedback, "Force Feedback", SubscribedBuffer::ForceFeedback);
  InitMappedBuffer(mGraphics, "Graphics", SubscribedBuffer::Graphics);
  InitMappedBuffer(mPitInfo, "Pit Info", SubscribedBuffer::PitInfo);
  InitMappedBuffer(mWeather, "Weather", SubscribedBuffer::Weather);
  InitMappedInputBuffer(mHWControl, "HWControl");
  InitMappedInputBuffer(mWeatherControl, "Weather control");
  InitMappedInputBuffer(mRulesControl, "Rules control");
  InitMappedInputBuffer(mPluginControl, "Plugin control");
  
  // Extended buffer is initialized last and is an indicator of initialization completed.
  InitMappedBuffer(mExtended, "Extended", SubscribedBuffer::All);
