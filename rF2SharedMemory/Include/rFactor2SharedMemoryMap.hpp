...
private:
  ...
  public:
    ...
    {
      auto const id = (std::max)(info.mID, 0L) % rF2Extended::MAX_MAPPED_IDS;
      ...
      if (info.mLastImpactET > dti.mLastPitStopET  // Is this new impact since last pit stop?
        ...
        td.mMaxImpactMagnitude = (std::max)(td.mMaxImpactMagnitude, info.mLastImpactMagnitude);
    ...
    void ProcessScoringUpdate(ScoringInfoV01 const& info)
    {
      for (int i = 0; i < info.mNumVehicles; ++i) {
        if (info.mVehicle[i].mPitState == static_cast<unsigned char>(rF2PitState::Stopped)) {
          ...
          auto const id = (std::max)(info.mVehicle[i].mID, 0L) % rF2Extended::MAX_MAPPED_IDS;
    ...
    void CaptureSessionTransition(rF2Scoring const& scoring)
    {
      ...
      auto const numScoringVehicles = (std::min)(scoring.mScoringInfo.mNumVehicles,
        static_cast<long>(rF2MappedBufferHeader::MAX_MAPPED_VEHICLES));
...
public:
  ...
  ///////////////////////////////////////////////////
    ...
    /*namespace SharedMemoryPlugin {
        Napi::Object SetExports(Napi::Env env, Napi::Object exports);
        Napi::Number Startup(const Napi::CallbackInfo& info);
  ...
  static Napi::Object SetExports(Napi::Env env, Napi::Object exports);
  static Napi::Number Init(const Napi::CallbackInfo& info);
  static Napi::Object GetTelemetry(const Napi::CallbackInfo& info);
  static Napi::Object GetScoring(const Napi::CallbackInfo& info);
  static Napi::Number Cleanup(const Napi::CallbackInfo& info);
